...
func (rooms *Rooms) FetchinRooms(hotelId uint, isAvailable, isBlocked, adminApproval bool, db *gorm.DB) ([]Rooms, error) {
	var roomss []Rooms
	if err := db.Preload("Cancellation").Preload("Hotels").Preload("RoomCategory").Where("hotels_id = ? AND is_available = ? AND is_blocked = ? AND admin_approval = ?", hotelId, isAvailable, isBlocked, adminApproval).Find(&roomss).Error; err != nil {
		return nil, err
	}
	return roomss, nil
}
