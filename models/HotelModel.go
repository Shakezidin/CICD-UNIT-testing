...
func (hotel *Hotels)FetchHotels(city string,isAvailab, isBlock,adminApproval bool,skip,limit int,db *gorm.DB)([]Hotels,error){
	var hotels []Hotels
	if err := db.Preload("HotelCategory").Offset(skip).Limit(limit).Where("city = ? AND is_available = ? AND is_block = ? AND admin_approval = ?", city, isAvailab, isBlock, adminApproval).Find(&hotels).Error; err != nil {
		return nil,err
	}
	return hotels,nil
}
